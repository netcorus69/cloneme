...
// Top lip — moves DOWN (in this mesh Y, top lip has lower Y value)
"  if(pos.y > 0.5 && pos.y < 1.2){\n"
"    float w = (pos.y - 0.5) / 0.7;\n"
"    pos.y -= uMouth * 0.8 * w;\n"   // ← changed += to -=
"  }\n"
// Bottom lip — moves UP
...
static void render(int m, int tick){
    ...
    if(g_head_vao && g_head_idx_count){
        ...
        mtrans(T, 0.0f, 0.0f, -1.0f);    // position
...
static void run_face_process(){
    ...
    if(!load_obj_to_vao("head.obj",&g_head_vao,&g_head_idx_count)){
    ...
    g_albedo_tex = load_texture("face.png");
    glUseProgram(g_prog);
